...
> **Примечание (ноябрь 2025)**: Desktop-клиент был мигрирован с Tauri на Electron из-за проблем производительности рендеринга SVG-графиков в WebKitGTK на Linux. Electron обеспечивает консистентную производительность благодаря встроенному Chromium с аппаратным ускорением. Подробности см. в [desktop/ELECTRON_MIGRATION.md](./desktop/ELECTRON_MIGRATION.md).
...
| ------------------- | --------------------------------------------- | ---------------------------- |
| Фреймворк           | **Electron**                                  | кроссплатформенная оболочка (MIT) |
| UI                  | **React + TypeScript + Tailwind + shadcn/ui** | интерфейс, карточки, таблицы |
| State Management    | **Zustand**                                   | управление состоянием        |
| Локальный кэш       | **LocalStorage**                              | персистентное хранилище      |
| Коммуникация        | **Axios**                                     | REST API клиент с JWT        |
